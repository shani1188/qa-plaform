...
const failureDetails = result.failures.length ? result.failures.slice(0, 10).map((item) => {
  const actual = item.actual.replace(/```/g, "'''");
  const reproduction = item.reproduction.map((step, index) => `${index + 1}. ${step}`).join("\n");
  const evidence = item.evidence.length ? item.evidence.map((entry) => `- ${entry}`).join("\n") : `- [Open workflow evidence](${result.run.runUrl})`;
  return `<details>\n<summary><strong>${item.severity.toUpperCase()} · ${item.suite}</strong> — ${item.title}</summary>\n\n**Expected**\n\n${item.expected}\n\n**Actual error**\n\n\`\`\`text\n${actual}\n\`\`\`\n\n**Reproduction**\n\n${reproduction}\n\n**Evidence captured**\n\n${evidence}\n\nFingerprint: \`${item.fingerprint}\`\n</details>`;
}).join("\n\n") : "No developer action is required.";
const omittedFailureCount = Math.max(0, result.failures.length - 10);
